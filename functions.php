...
function dbConnect(){
    global $db;
	$db=new mysqli("localhost","maulnad","password","db_data");
	return $db;
}
...
function getDataPengadaan(){
	$db=dbConnect();
	if($db->connect_errno==0){
		$sql= "SELECT nama_buku, nama_penerbit, stok FROM buku INNER JOIN penerbit ON buku.id_penerbit = penerbit.id_penerbit ORDER BY stok ASC;";
		$res=$db->query($sql);
		if($res){
			$data=$res->fetch_all(MYSQLI_ASSOC);
			$res->free();
